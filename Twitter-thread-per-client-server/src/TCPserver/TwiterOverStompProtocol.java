...
public class TwiterOverStompProtocol implements ServerProtocol {
	 ...
	 * @param CH the ch
	 */
	public TwiterOverStompProtocol(ConnectionHandler CH){
		this.CH=CH;
	}
	 ...
	 */
	public StompFrame processMessage(Object Object) {
		if (Object!=null) {return ((StompFrame)(Object)).Process(this.Dispatcher);}
		return null;
	}
	...
	public boolean isEnd(Object msg) {
		if	(msg.toString().contains("DISCONNECT")) this.ShouldClose=true;
	...
	public void connectionTerminated() {
		if (this.user!=null) {
			this.user.logOUT();
			this.LOGGER.warning("The user"+this.user.getName()+" disconnecting from the server without a disconnect command");
			this.user=null;}
	...
	public StompFrame Processframe(UNSUBSCRIBE msg){
		if (this.user!=null && this.user.isLoggedIN()){
			...
			if (user!=null){
				...
				this.LOGGER.warning(this.user.getName()+"request following the user , but the user isn't registered on this server ");
